...
def is_date_in_range(date_str, since_date_str):
    """날짜가 주어진 범위 내에 있는지 확인"""
    if not date_str:
        return False

    # ISO 형식 날짜(2023-04-14T15:30:45Z)에서 날짜 부분만 추출
    date_only = date_str.split('T')[0]

    # 날짜 비교
    return date_only >= since_date_str


def filter_prs_by_date(prs, since_date_str):
    """PR 목록에서 검토 기간 내의 PR만 필터링"""
    filtered_prs = []
    ...
    for pr in prs:
        ...
        # 머지된 PR은 머지 날짜가 검토 기간 내인 경우만 포함
        elif pr['mergedAt'] and is_date_in_range(pr['mergedAt'], since_date_str):
            filtered_prs.append(pr)
        # 닫힌 PR(머지되지 않음)은 업데이트 날짜가 검토 기간 내인 경우만 포함
        elif pr['state'] == 'CLOSED' and not pr['mergedAt'] and is_date_in_range(pr['updatedAt'], since_date_str):
            filtered_prs.append(pr)
...
def generate_summary_section(repo_full_name, user_data, all_prs, days_ago_str, today_str):
    ...
    # 전체 통계 계산 (검토 기간 내 머지된 PR만 포함)
    total_prs = len(all_prs)
    pr_open = sum(1 for pr in all_prs.values() if pr['state'] == 'OPEN')
    pr_merged = sum(1 for pr in all_prs.values() if pr['mergedAt'] and is_date_in_range(pr['mergedAt'], days_ago_str))
    pr_closed = sum(1 for pr in all_prs.values() if
                    pr['state'] == 'CLOSED' and not pr['mergedAt'] and is_date_in_range(pr['updatedAt'], days_ago_str))
    ...
    summary += f"- **PR 현황**: 총 {pr_open + pr_merged + pr_closed}개 (진행 중: {pr_open}개, 머지됨: {pr_merged}개, 닫힘: {pr_closed}개)\n"
    summary += f"- **작업 중인 브랜치**: {total_wip_branches}개 (커밋: {total_wip_commits}개)\n\n"

    # 사용자별 활동 랭킹
    summary += f"### 👥 사용자별 활동 랭킹\n\n"
    summary += "| 순위 | 사용자 | PR 수 | 머지된 PR | 진행 중 PR | 작업 중인 브랜치 커밋 |\n"
    summary += "| ---- | ------ | ----- | --------- | ---------- | -------------------- |\n"

    # 랭킹을 위한 사용자 데이터 준비
    user_ranking = []
    for username in user_data:
        # PR 통계 계산 - 검토 기간 내 관련된 PR만 계산
        user_merged_prs = sum(1 for pr in user_data[username]['filtered_prs'] if pr['mergedAt'])
        user_open_prs = sum(1 for pr in user_data[username]['filtered_prs'] if pr['state'] == 'OPEN')
        user_closed_prs = sum(
            1 for pr in user_data[username]['filtered_prs'] if pr['state'] == 'CLOSED' and not pr['mergedAt'])

        # 총 PR 수는 관련 PR의 합으로 계산 (기간 내 머지 + 오픈 + 기간 내 닫힘)
        user_pr_count = user_merged_prs + user_open_prs + user_closed_prs

        user_wip_commits = user_data[username]['wip_commit_count']

        user_ranking.append({
            'username': username,
            'pr_count': user_pr_count,
            'merged_prs': user_merged_prs,
            'open_prs': user_open_prs,
            'wip_commits': user_wip_commits
        })

    # 1차: PR 수, 2차: 작업 중인 브랜치 커밋 수로 정렬
    user_ranking.sort(key=lambda x: (x['pr_count'], x['wip_commits']), reverse=True)

    # 랭킹 표시
    for rank, user in enumerate(user_ranking, 1):
        username = user['username']
        pr_count = user['pr_count']
        merged_prs = user['merged_prs']
        open_prs = user['open_prs']
        wip_commits = user['wip_commits']

        summary += f"| {rank} | @{username} | {pr_count} | {merged_prs} | {open_prs} | {wip_commits} |\n"
    ...
    # 최근 머지된 PR 목록 (검토 기간 내 머지된 PR만 표시)
    summary += f"#### ✅ 최근 머지된 PR\n\n"
    merged_prs = []
    for pr in all_prs.values():
        # 검토 기간 내에 머지된 PR만 필터링
        if pr['mergedAt'] and is_date_in_range(pr['mergedAt'], days_ago_str):
            merged_prs.append({
...
def generate_markdown_report(repo_full_name, user_handles, git_repo, days):
    ...
    # 사용자별 데이터를 저장할 딕셔너리
    user_data = {username: {
        'prs': [],  # 사용자가 작성하거나 할당된 모든 PR
        'filtered_prs': [],  # 필터링된 PR (기간 내)
    ...
    for username in user_handles:
        ...
        # 기간 내 PR만 필터링
        user_data[username]['filtered_prs'] = filter_prs_by_date(user_data[username]['prs'], days_ago_str)
    ...
    for username in user_handles:
        ...
        filtered_prs = user_data[username]['filtered_prs']

        if filtered_prs:
            # PR을 상태별로 분류
            open_prs = [pr for pr in filtered_prs if pr['state'] == 'OPEN']
            merged_prs = [pr for pr in filtered_prs if pr['mergedAt']]
            closed_prs = [pr for pr in filtered_prs if pr['state'] == 'CLOSED' and not pr['mergedAt']]

            # 진행 중인 PR
            if open_prs:
                user_section += f"#### 진행 중인 PR ({len(open_prs)}개)\n\n"
                user_section += "| PR | 제목 | 마지막 업데이트 |\n"
                user_section += "| -- | ---- | -------------- |\n"
                ...
                for pr in sorted(open_prs, key=lambda x: x['updatedAt'], reverse=True):
                    ...
                    user_section += f"| [#{pr_number}](<{pr_url}>) | {pr_title} | {pr_updated} |\n"
            ...
            if merged_prs:
                ...
                for pr in sorted(merged_prs, key=lambda x: x['mergedAt'], reverse=True):
                    ...
                    user_section += f"| [#{pr_number}](<{pr_url}>) | {pr_title} | {pr_merged} |\n"
